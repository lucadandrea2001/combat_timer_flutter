...
class _TimerScreenState extends State<TimerScreen> {
  ...
  void _startTimer() {
    ...
    _currentCycle = 1;
    _isWorking = true;
    _nextBeep = _random.nextInt(4) + 2;  // 2–5 secondi
    _nextGong = _random.nextInt(5) + 4;  // 4–8 secondi
      ...
      setState(() {
        ...
        if (_secondsLeft <= 0) {
          if (_isWorking) {
            ...
            _gongPlayer.play(AssetSource('sounds/gong.wav')); // segnale riposo
          } else {
            if (_currentCycle < _cycles) {
              _currentCycle++;
              _isWorking = true;
              _secondsLeft = _workDuration;
              _gongPlayer.play(AssetSource('sounds/gong.wav')); // inizio round successivo
              _nextBeep = _random.nextInt(4) + 2;
              _nextGong = _random.nextInt(5) + 4;
            } else {
              _timer?.cancel();
              _gongPlayer.play(AssetSource('sounds/gong.wav')); // segnale finale
            }
          }
        }
      });
            ...
            Row(
              ...
              children: [
                ...
                ElevatedButton(
                  ...
                  child: Icon(Icons.pause, size: 36),
                ),
                SizedBox(width: 20),
                ElevatedButton(
                  onPressed: () {
